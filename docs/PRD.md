...
| v1.5 | 2026-06-19 | 前台文案和产品定位回归个人知识库与学校事项板，访客可见页面不展示内部权限枚举、认证技术栈或数据库字段 |
...
| Unlisted | 持链接访问内容，不进入 `/wiki` 或 `/school` 列表，通过 `/share/[token]` 访问，支持访问码、过期、撤销和访问统计 |
| PostShare | 单篇 unlisted 内容的受控分享记录，包含随机 token、访问码 hash、过期时间、撤销状态和访问统计 |
| Project | 项目标签，不是完整项目管理模块，只用于归类内容 |
| Attachment | 关联到 Post 的附件元数据，文件本体保存到服务端本地目录，下载统一经过受控接口鉴权 |
| 访客可见文案 | 面向普通访问者展示的首页、公开知识库、学校事项、分享页和登录页说明文案，应使用内容价值语言，不展示内部权限枚举、认证技术栈或数据库字段 |

## 1. 项目背景

用户日常同时处理技术学习、项目记录、个人博客素材、运维笔记、课程通知、作业截止、考试安排和小组任务等信息。

这些信息目前分散在聊天记录、截图、文档、备忘录、NAS、Git 仓库和临时文件中，存在以下问题：

- 出门在外时资料难以统一检索。
- 技术笔记和项目复盘难以沉淀成长期可阅读的个人知识库。
- 学校通知、作业、考试和截止时间缺少一个清晰的事项板。
- 私密草稿、未整理资料和可公开文章容易混在一起。
- 项目记录、学习资料、附件和参考链接缺少统一分类、标签和归档入口。

TekFlow V1.0.0 先解决个人高频使用场景，聚焦公开知识阅读、学校事项查看和私密后台整理，不做复杂项目管理，也不做 AI 方案生成器。

## 2. 项目目标

本项目要解决个人用户在技术沉淀、学校事项和项目资料管理中的统一记录、检索、阅读和整理问题。

项目目标包括：

- 管理员可以登录后台维护 Markdown 文章、草稿、学习资料和学校事项。
- 内容可以按分类、标签、项目标签、类型和发布状态组织。
- 公开文章可以进入 `/wiki`，形成可阅读的个人知识库。
- 课程、作业、考试和截止时间可以进入 `/school`，形成学校事项板。
- 私密草稿和未整理资料只在管理后台维护。
- 附件可以关联到内容，方便归档截图、PDF、Office、Markdown/TXT 和压缩包等资料。
...
- 管理员能登录后台并新建一篇内容。
- 公开文章能在 `/wiki` 查看。
- 学校事项能在 `/school` 按时间紧急程度查看。
- 私下分享文章能通过指定链接访问，但不出现在公开列表。
- 私密草稿不会出现在访客可见页面。
- 附件不会脱离所属内容被错误公开。

## 3. 项目定位

TekFlow 定位为个人知识库、公开博客内容入口和学校事项板，不是正式商业级协作平台。

| 维度 | 说明 |
|---|---|
| 项目类型 | Web 前端 + Java 后端 + MySQL + 本地附件存储 |
| 复杂度 | 中等，优先保证个人可用、可运行、可讲解 |
| 核心亮点 | Markdown 写作、公开知识库、学校事项板、私密草稿后台、附件归档 |
...
| `/share/[token]` 受控私下分享内容，支持访问码、过期、撤销和统计 | 多人分享策略、访问审批流、分享用户画像 |
...
| 角色 | 主要目标 | 第一阶段说明 |
|---|---|---|
| 管理员 | 维护文章、学校事项、附件和草稿 | 登录后台，创建、查看、编辑、删除、发布全部内容 |
| 知识库读者 | 阅读公开技术笔记、学习资料和项目复盘 | 访问 `/wiki` 和公开文章详情 |
| 学校事项查看者 | 查看课程、作业、考试和截止时间 | 访问 `/school` 和学校事项详情 |
| 分享链接访问者 | 通过指定链接查看单篇分享内容 | 访问 `/share/[token]`，必要时输入访问码，不依赖公开列表入口 |

V1.0.0 第一版只有管理员账号是必需项。用户表允许后续扩展多个管理员，但第一阶段不做管理员管理页面。同学用户体系暂缓；学校事项页先按访客可查看的事项板实现。

用户画像：

| 用户 | 场景 | 目标 |
|---|---|---|
| 个人管理员 | 记录技术经验、学习资料、学校通知、项目复盘 | 统一写入、分类、检索和发布 |
| 知识库读者 | 浏览公开技术文章和方法论 | 阅读已经整理和脱敏的公开内容 |
| 学校事项查看者 | 查看课程通知、作业截止、考试安排 | 按时间紧急程度查看事项 |
| 分享链接访问者 | 从私下分享链接打开内容 | 查看不出现在公开列表的单篇资料 |
| 答辩/演示观看者 | 验证项目是否形成完整闭环 | 观察后台写入、前台展示、筛选和附件归档 |
...
|---|---|---|
| 技术文章沉淀 | 写在本地文件、聊天记录或临时文档中 | 统一写入内容后台，公开文章进入 `/wiki` |
| 学校通知记录 | 截图、群聊、备忘录分散 | School Notice 结构化记录时间、课程、地点、截止日期 |
| 草稿与公开内容 | 靠人工记忆判断是否能公开 | 发布前明确选择展示范围，访客页面只呈现适合公开阅读的内容 |
| 附件管理 | 文件和正文分离，查找困难 | Attachment 关联 Post，随正文一起归档 |
...
| FR-ADM-010 | 基础设置页 | P1 | `/dashboard/settings` 展示账号信息、工作台概览、上传限制和发布提醒，不做复杂可写配置 |
...
| 编号 | 功能 | 优先级 | 说明 |
|---|---|---|---|
| FR-SHARE-001 | unlisted 详情页 | P0 | `/share/[token]` 展示已开启分享、未撤销、未过期的 `visibility = unlisted` 且 `status = published` 内容 |
| FR-SHARE-002 | unlisted 列表排除 | P0 | unlisted 内容不出现在 `/wiki`、`/school` 或首页公开列表中 |
| FR-SHARE-003 | unlisted 附件访问 | P0 | unlisted 附件通过分享 token、访问码、过期时间和撤销状态校验后访问 |
| FR-SHARE-004 | 分享管理 | P0 | 后台可开启分享、设置/清除访问码、设置过期时间、撤销分享和重新生成链接 |
| FR-SHARE-005 | 分享统计 | P1 | 后台展示成功打开详情次数、附件下载次数和最后访问时间 |
...
- `/share/[token]` 只能展示已开启分享、未撤销、未过期且校验通过的 unlisted published 内容。
- 附件权限必须跟随所属 Post，不能通过静态公开路径绕过受控接口。
- 客户端不得依赖中文 `msg` 判断业务逻辑。
- V1.0.0 功能边界必须清楚，不做过度扩展。
- 访客可见页面和登录页说明不得展示内部权限枚举、认证技术栈、数据库字段或实现型安全说明；这些内容只保留在技术实现、接口文档和安全规则中。
...
| US-006 | 作为链接访问者，我希望通过私下分享链接查看内容，以便访问不公开列出的资料 | `/share/[token]` 能在校验访问码和过期状态后打开 unlisted published 内容，且该内容不出现在 `/wiki` 或 `/school` |
...
- unlisted 分享开启、撤销、重新生成、门禁信息和详情打开接口。
- 分类、标签、项目标签基础 CRUD。
- 附件上传、元数据查询和受保护下载。
...
| FR-SHARE-001 | token、可选访问码 | unlisted Post 详情 | 内容已发布、unlisted、分享已开启且未过期未撤销 | token 不存在、访问码错误、已过期、已撤销、未发布、非 unlisted |
...
| 发布 unlisted | 需要标题、slug、类型、状态、可见性和非空正文；发布后管理员开启分享并通过 `/share/[token]` 访问 |
...
|---|---|---|---|---|---|
| `/` 首页 | 知识库读者 / 学校事项查看者 / 管理员 | 提供 TekFlow 内容入口 | 个人知识库介绍、`/wiki`、`/school`、`/login`、最新公开内容、近期学校事项 | 是 | 无公开内容、加载失败 |
| `/login` 登录页 | 管理员 | 登录后台 | 管理后台说明、username、密码、登录按钮 | 是 | 空输入、密码错误、登录中 |
...
| `/dashboard/settings` 设置 | 管理员 | 查看账号和工作台概览 | 账号信息、附件上限、写作方式、学校事项提醒 | 是 | 配置缺失、加载失败 |
| `/wiki` 公开知识库 | 知识库读者 | 浏览公开知识内容 | 列表、分类、标签、搜索 | 是 | 无内容、加载失败 |
| `/wiki/[slug]` 公开文章详情 | 游客 | 阅读公开文章 | 标题、摘要、正文、标签、附件 | 是 | 不存在、未发布、非 public |
| `/share/[token]` 链接访问详情 | 链接访问者 | 查看受控 unlisted 内容 | 分享门禁、访问码输入、标题、摘要、正文、标签、附件 | 是 | 不存在、访问码错误、已过期、已撤销、未发布、非 unlisted |
| `/school` 学校事项板 | 学校事项查看者 | 查看学校事项 | 即将截止、今日、本周、已过期、全部通知视图 | 是 | 无事项、已过期 |
| `/school/[slug]` 学校通知详情 | 学校事项查看者 | 查看单条事项 | 时间、地点、课程、老师、正文、附件 | 是 | 不存在、未发布、非 school |
...
flowchart TD
    ...
    J -->|private| M[仅后台可见]
    J -->|unlisted| N[后台开启分享后通过 /share/token 持链接访问]
```
...
flowchart TD
    ...
    E -->|是| F
    E -->|否| G[提示访问码错误]
    C -->|否| H[显示过期、撤销或不可访问提示]
    F --> I[展示 Markdown 详情和分享附件下载入口]
```
...
| 链接访问 | `unlisted` | 不在 `/wiki`、`/school` 或首页公开列表 | 持 token 访问 `/share/[token]`，可选访问码 | 通过分享 token、访问码、过期和撤销状态校验后访问 |
...
| `visibility = unlisted` | 只允许通过已开启的 `/share/[token]` 访问，不进入列表 |
...
| token | String | 随机分享 token，用于 `/share/[token]` |
| access_code_hash | String | 可选访问码 hash，不保存明文访问码 |
| expires_at | DateTime | 过期时间，null 表示永不过期 |
| status | String | 分享状态，`active` 或 `revoked` |
| access_count | Long | 成功打开详情次数 |
| attachment_download_count | Long | 分享附件下载次数 |
| last_accessed_at | DateTime | 最后成功打开详情时间 |
...
- public/school 附件访问统一经过 `/api/v1/attachments/{id}`；unlisted 分享附件经过 `/api/v1/share/posts/{token}/attachments/{id}`，后端根据分享 token、访问码、过期时间、撤销状态和所属 Post 判断是否允许访问。
...
- unlisted 内容只允许通过已开启的 `/share/[token]` 访问，不进入公开列表；撤销、过期或访问码错误时不能访问正文或附件。
...
- 首页、公开知识库、学校事项、分享页和登录页说明必须使用访客能理解的内容语言，不展示 `NextAuth`、`JWT`、`visibility`、`status`、`deleted_at`、`private/public/school/unlisted` 等内部实现或字段。
...
- public/school 附件下载和预览通过 `/api/v1/attachments/{id}`；unlisted 分享附件通过 `/api/v1/share/posts/{token}/attachments/{id}`；不提供静态公开附件路径。
...
| 查询分享 | GET | `/api/v1/admin/posts/{id}/share` | 管理员 | 查询分享 token、访问码状态、过期时间和统计 | 后台编辑页加载 |
| 开启/更新分享 | POST | `/api/v1/admin/posts/{id}/share` | 管理员 | 为 unlisted published 内容开启或更新分享设置 | 分享卡片刷新 |
| 撤销分享 | DELETE | `/api/v1/admin/posts/{id}/share` | 管理员 | 撤销当前分享，使旧链接失效 | 分享卡片刷新 |
| 重新生成分享 | POST | `/api/v1/admin/posts/{id}/share/rotate` | 管理员 | 生成新 token，使旧 token 失效 | 分享卡片刷新 |
| 公开列表 | GET | `/api/v1/wiki/posts` | 游客 | 查询 public published 内容 | `/wiki` 加载 |
| 公开详情 | GET | `/api/v1/wiki/posts/{slug}` | 游客 | 查询 public 详情 | `/wiki/[slug]` 加载 |
| 分享门禁信息 | GET | `/api/v1/share/posts/{token}/meta` | 链接访问者 | 查询是否需要访问码、是否过期或撤销 | `/share/[token]` 加载 |
| 打开分享内容 | POST | `/api/v1/share/posts/{token}/open` | 链接访问者 | 校验 token、访问码、过期和撤销状态，成功后计数并返回详情 | `/share/[token]` 加载 |
...
| 分享附件访问 | POST | `/api/v1/share/posts/{token}/attachments/{id}` | 链接访问者 | 校验分享 token 和访问码后下载 unlisted 附件并计数 | 分享详情页下载 |
...
- `/share/[token]` 只显示已开启分享、未撤销、未过期且校验通过的 unlisted + published 内容。
- 管理员可以为 unlisted published 内容开启分享、设置/清除访问码、设置过期时间、撤销分享和重新生成链接。
- 设置访问码后，访客必须输入正确访问码才能打开正文和附件。
- 分享过期或撤销后，旧链接和分享附件下载都不可访问。
- 后台可以展示成功打开详情次数、附件下载次数和最后访问时间。
- unlisted 内容不会出现在 `/wiki`、`/school` 或首页最新公开内容。
- draft、archived、private、public、school 内容不能通过 `/share/[token]` 错误访问。
...
| 8 | 新建 unlisted 内容并开启分享 | `/share/[token]` 可访问，但不出现在 `/wiki` 或 `/school` |
...
| AC-011 unlisted 链接访问 | 内容为 unlisted + published 且分享已开启 | 打开 `/share/[token]` 并通过访问码校验 | 显示该内容并计入成功访问次数 |
...
| AC-019 分享撤销 | unlisted 分享已开启 | 后台撤销分享后打开旧链接 | 正文和附件均不可访问 |
| AC-020 分享重新生成 | unlisted 分享已开启 | 后台重新生成分享链接 | 旧 token 失效，新 token 可访问 |
