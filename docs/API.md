...
| `unlisted` | 不进入公开列表 | 持 token 访问 `/share/[token]`，可选访问码 | 分享 token、过期时间、撤销状态和访问码校验通过后允许访问 |

补充规则：

- 所有公开侧查询只返回 `status = published` 且未软删除的内容。
- `type = school_notice` 必须绑定 `visibility = school`。
- `/share/[token]` 只允许访问已开启分享、未撤销、未过期的 `unlisted + published` 内容。
- unlisted 分享可设置访问码和过期时间；成功打开详情计入访问次数，分享附件下载另计下载次数。
- public/school 附件通过 `/api/v1/attachments/{id}` 受控下载；unlisted 分享附件通过 `/api/v1/share/posts/{token}/attachments/{id}` 校验后下载；不提供静态公开附件路径。
...
| 查询分享 | GET | `/api/v1/admin/posts/{id}/share` | 管理员 | 查询分享 token、访问码状态、过期时间和访问统计 |
| 开启/更新分享 | POST | `/api/v1/admin/posts/{id}/share` | 管理员 | 为 unlisted published 内容开启或更新分享，默认 7 天过期 |
| 撤销分享 | DELETE | `/api/v1/admin/posts/{id}/share` | 管理员 | 撤销当前分享，使 token 和分享附件访问失效 |
| 重新生成分享 | POST | `/api/v1/admin/posts/{id}/share/rotate` | 管理员 | 重新生成 token，使旧分享链接失效 |
...
| 分享门禁信息 | GET | `/api/v1/share/posts/{token}/meta` | 链接访问者 | 查询分享是否需要访问码、是否过期或撤销；不计访问次数 |
| 打开分享内容 | POST | `/api/v1/share/posts/{token}/open` | 链接访问者 | 校验 token、访问码、过期和撤销状态，成功后返回内容详情并计数 |
...
| 分享附件访问 | POST | `/api/v1/share/posts/{token}/attachments/{id}` | 链接访问者 | 校验分享 token 和访问码后下载 unlisted 附件并计数 |
...
| 发布 unlisted | 正文必须非空；发布后管理员需要开启分享，访客通过 `/share/[token]` 访问 |
...
| 分享访问码 | 可选；设置后只保存 hash，不明文存储 |
| 分享过期时间 | 新分享默认 7 天后过期；管理员可设置自定义时间或永不过期 |
